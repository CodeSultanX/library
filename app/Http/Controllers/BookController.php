...
class BookController extends Controller
{
    ...
    {
        $items = $request->items ?? $variable = GlobalVariable::findOrFail(4)->value;
    ...
    public function recommend(Request $request, BookService $bookService)
    {

        $book_request_url = env('BOOK_RECOM_URI') . '/books/recomends?user_id=' . \Auth::id();
        $books_id = json_decode(file_get_contents($book_request_url), true)['data'];
        $books = Book::with('cover', 'authors', 'categories', 'rent')->whereIn('id', $books_id)->latest('id')->get();
        ...
        return view('pages.books.recommend_books', compact('books',
            'count', 'authors', 'categories', 'error', 'selected_a',
            'selected_c', 'id_a', 'id_c', 'error', 'show',
        ));
    }
