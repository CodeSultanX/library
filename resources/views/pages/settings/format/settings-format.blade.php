...
<section class="w-screen h-screen pl-[80px] py-4 text-gray-700">
    ...
    <div class="heading mt-[7px]">
        <div class="heading mt-[7px]" style="margin-top: 10px">
            <h1 style="font-size: 30px"
                ...
                {{-- Format update flash message --}}
                @if (session()->has('format-updated'))
                <script>
                    swal({
                        title: "@lang('Uspješno!')",
    ...
    <div style="display:none;" id="myDiv">
        ...
        <div class="inline-block min-w-full px-[50px] pt-3 align-middle bg-white rounded-bl-lg rounded-br-lg shadow-dashboard">
            ...
            <button type="submit"
                    class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE] button delete-all-formats"
                    data-url="">
                <div class="icon">
                    <svg class="top">
                        <use xlink:href="#top">
                    ...
                    <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                        ...
                        <div
                            ...
                            <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                ...
                                <div class="py-1">
                                    <a href="{{route('edit-format', $format->name)}}"
                                       tabindex="0"
                                    ...
                                    <button
                                            data-id="{{ $format->id }}"
                                            data-action="{{ route('formats.destroy', $format->id) }}"
                                            onclick="deleteConfirmation({{$format->id}})"
...
<script type="text/javascript">
    function deleteConfirmation(id) {
        swal({
            ...
            if (e.value === true) {
                ...
                swal(
                    'Uspješno!',
                    'Format je uspješno izbrisan.',
                    'success'
                ).then(function () {
                    window.location.href = "/podesavanja/formati";
                });
                $.ajax({
                    type: 'POST',
                    url: "{{url('/podesavanja/formati')}}/" + id,
    ...
    // Script for edit format profile top header
    function editFormat() {
        var name = $('#check:checked').val();
        window.location.href = "/podesavanja/izmijeni-format/" + name;
    }
</script>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.delete-all-formats').on('click', function (e) {
            var idsArr = [];
