...
@section('title')
    ...
    <!-- Title -->
    <title>{{__('Кітапқа тапсырыс беру | Online biblioteka')}}</title>
    ...
    <section class="w-screen h-screen pl-[80px] pb-2 text-gray-700">
                                ...
                                <div class="py-1">
                                    ...
                                    <form action="{{route('destroy-book', $book->id)}}"
                                        ...
                                        <button style="outline: none" type="submit"
                                            ...
                                            <span class="px-4 py-0">Кітапты жою</span>
                                        </button>
                                    </form>
                                </div>
        ...
        <div class="scroll height-content section-content">
            ...
            <form class="text-gray-700"
                  action="{{route('store-reserve-book', $book->id)}}" method="POST">
                @csrf
                @method('POST')

                <div class="flex flex-row ml-[30px]">
                    <div class="w-[50%] mb-[100px]">
                        <h3 class="mt-[20px] mb-[10px]">Кітап

                            {{-- Session message for successfully sent reservation
                            request --}}
                            @if (session()->has('reservation-sent'))
                                <div id="hideDiv"
                                    ...
                                    <div>
                                        <span class="font-medium">Сәтті!</span>
                            ...
                            @if (session()->has('reservation-failed'))
                                <div id="hideDiv"
                                     class="flex p-3 mt-2 mb-1 text-sm text-red-700 bg-red-200 rounded-lg dark:bg-red-200 dark:text-red-800"
                                     role="alert">
                                    <svg aria-hidden="true"
                                         class="flex-shrink-0 inline w-5 h-5 mr-3"
                                         fill="currentColor" viewBox="0 0 20 20"
                                    ...
                                    <div>
                                        <span class="font-medium">Кате!</span>
                                        {{session('reservation-failed')}}
                                    </div>
                                </div>
                                <div class="flex p-4 mt-2 mb-1 text-sm text-white bg-blue-700 rounded-lg"
                                     role="alert">
                                    <svg aria-hidden="true"
                                         class="flex-shrink-0 inline w-5 h-5 mr-3"
                                         fill="currentColor" viewBox="0 0 20 20"
                                    ...
                                    <span class="sr-only">Info</span>
                                    <div>
                                        <span class="font-medium">Инфо!</span>
                                        Мүмкін болу үшін кітапханашыға хабарласыңыз
                                        бір уақытта бірнеше кітапқа тапсырыс беріңіз (өзгерту
                                        пакет).
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="mt-[20px]">
                            ...
                            Auth::user()->type->id == 3)
                                <span>Кітап сақталған студентті таңдаңыз <span
                                        class="text-red-500">* @error('reservationMadeFor_user_id'){{$message}} @enderror</span></span>
                                <select
                                    ...
                                    @else
                                        ...
                                        <select
                                            ...
                                            id="reservationMadeFor_user_id"
                                            for="reservationMadeFor_user_id"
                                            name="reservationMadeFor_user_id">
