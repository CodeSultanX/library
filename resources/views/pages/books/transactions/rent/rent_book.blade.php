...
<!-- Title -->
<title>{{__('Izdaj knjigu | Online biblioteka')}}</title>
...
<section class="w-screen h-screen pl-[80px] pb-2 text-gray-700">
            ...
            <div class="pt-[24px] mr-[30px]">
                ...
                <div
                    ...
                    <div class="absolute right-0 w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                        ...
                        <div class="py-1">
                            <a href="{{route('edit-book', $book->title)}}"
                               tabindex="0"
                               class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                               role="menuitem">
                                <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                <span class="px-4 py-0">Кітапты өзгерту</span>
                            </a>
                            <form action="{{route('destroy-book', $book->id)}}"
                                  method="POST">
                                @csrf @honeypot
                                @method('DELETE')
    ...
    <div class="scroll height-content section-content">
        <form class="text-gray-700"
              action="{{route('store-rent-book', $book->id)}}" method="POST">
            {{ csrf_field() }}
            @method('POST')
            <div class="flex flex-row ml-[30px]">
                <div class="w-[50%] mb-[100px] mr-[100px]">
                    <h3 class="mt-[20px] mb-[10px]">Кітап шығару</h3>
                    <div class="mt-[20px]">
                        <span>Izaberi učenika koji zadužuje knjigu <span
                                    class="text-red-500">* @error('borrow_user_id'){{$message}} @enderror</span></span>
                        <select
                                ...
                                onclick="clearErrorsUcenikIzdavanje()">
                    ...
                    <div class="mt-[20px] flex justify-between w-[90%]">
                        <div class="w-[50%]">
                            <p>Datum izdavanja <span
                                        class="text-red-500">*</span></p>
                            <label class="text-gray-700" for="date">
                                <input type="date" value="{{$current_one}}"
                                       name="issue_date" id="datumIzdavanja"
                ...
                <div class="w-[50%] mb-[100px]">
                    <div class="border-[1px] border-[#e4dfdf] w-[360px] mt-[75px]">
                        <h2 class="mt-[20px] ml-[30px]">Саны</h2>
                        <div class="ml-[30px] mr-[70px] mt-[20px] flex flex-row justify-between">
                            <div class="text-gray-500 ">
                                <p>Сіздің қолыңызда:</p>
                                <p class="mt-[20px]">Сақталған:</p>
                                <p class="mt-[20px]">Берілген:</p>
                                <p class="mt-[20px]">Овердрафтта:</p>
                                <p class="mt-[20px]">Жалпы саны:</p>
                            </div>
                            <div class="text-center pb-[30px]">
                                <p class=" bg-green-200 text-green-700 rounded-[10px] px-[6px] py-[2px] text-[14px]">
                                    ...
                                    $book->reserved_count)) : "0"}}
                                    @php
                                    if ($book->quantity_count -
                                    ($book->rented_count +
                                    $book->reserved_count) % 10 == 1 ||
                                    $book->quantity_count - ($book->rented_count
                                    + $book->reserved_count) % 10 == 11 ||
                                    $book->quantity_count - ($book->rented_count
                                    + $book->reserved_count) == 1) {
                                    echo "primjerak";
                                    } elseif (($book->quantity_count -
                                    ($book->rented_count +
                                    $book->reserved_count)) % 10 == 2 ||
                                    ($book->quantity_count -
                                    ($book->rented_count +
                                    $book->reserved_count)) % 10 == 3 ||
                                    ($book->quantity_count -
                                    ($book->rented_count +
                                    $book->reserved_count)) % 10 == 4) {
                                    echo "primjerka";
                                    } else {
                                    echo "primjeraka";
                                    }
                                ...
                                <a href="{{route('active-reservations')}}">
                                    <p class=" mt-[16px] bg-yellow-200 text-yellow-700 rounded-[10px] px-[6px] py-[2px] text-[14px]">
                                        ...
                                        $book->reserved_count : "0"}}
                                        @php
                                        if ($book->reserved_count % 10 == 1 ||
                                        $book->reserved_count % 10 == 11 ||
                                        $book->reserved_count == 1) {
                                        echo "primjerak";
                                        } elseif ($book->reserved_count % 10 ==
                                        2 || $book->reserved_count % 10 == 3 ||
                                        $book->reserved_count % 10 == 4) {
                                        echo "primjerka";
                                        } else {
                                        echo "primjeraka";
                                ...
                                <a href="{{route('rented-books')}}">
                                    <p class=" mt-[16px] bg-blue-200 text-blue-800 rounded-[10px] px-[6px] py-[2px] text-[14px]">
                                        ...
                                        $book->rented_count : "0"}}
                                        @php
                                        if ($book->rented_count % 10 == 1 ||
                                        $book->rented_count % 10 == 11) {
                                        echo "primjerak";
                                        } elseif ($book->rented_count % 10 == 2
                                        || $book->rented_count % 10 == 3 ||
                                        $book->rented_count % 10 == 4) {
                                        echo "primjerka";
                                        } else {
                                        echo "primjeraka";
                                        }
                                ...
                                <p class=" mt-[16px] border-[1px] border-green-700 text-green-700 rounded-[10px] px-[6px] py-[2px] text-[14px]">
                                    ...
                                    $book->quantity_count : "0"}}
                                    @php
                                    if ($book->quantity_count % 10 == 1 &&
                                    $book->quantity_count != 11) {
                                    echo "primjerak";
                                    } elseif ($book->quantity_count % 10 == 2 ||
                                    $book->quantity_count % 10 == 3 ||
                                    $book->quantity_count % 10 == 4) {
                                    echo "primjerka";
                                    } else {
                                    echo "шт";
                                    }
                ...
                <div class="flex flex-row">
                    <div class="inline-block w-full text-right py-[7px] mr-[100px] text-white">
                        ...
                        <button id="izdajKnjigu" type="submit"
                                class="mr-[15px] mb-[10px] btn-animation shadow-lg w-[150px] disabled:opacity-50 focus:outline-none text-sm py-2.5 px-5 transition duration-300 ease-in rounded-[5px] hover:bg-[#46A149] bg-[#4CAF50]">
