...
@section('content')
        ...
        <section class="w-screen h-screen py-4 pl-[80px] text-[#333333]">
                ...
                <h1 style="font-size: 30px"
                    ...
                    @if (session()->has('success-book'))
                        <script>
                            swal({
                                title: "@lang('Uspješno!')",
                                text: "@lang('Uspješno ste dodali knjigu!')",
                                timer: 2500,
                                type: "success",
                            });
                        </script>
                    ...
                    @if (session()->has('book-deleted'))
                        <script>
                            swal({
                                title: "@lang('Uspješno!')",
                                text: "@lang('Uspješno ste izbrisali knjigu!')",
                                timer: 2500,
                                type: "success",
                            });
                        </script>
                ...
                <div class="flex items-center justify-between px-[30px] py-4 space-x-3 rounded-lg">
                    ...
                    @if ($books->count())
                        <div class="flex items-center">
                            ...
                            <input type="search" name="trazeno"
                                   value="{{$searched_book}}"
            ...
            <div class="px-[30px] pt-2 bg-white">
                <div class="w-full mt-2">
                    ...
                    && $books->count())
                        <button type="submit"
                                class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE] button delete-all-books"
                                data-url="">
                            <div class="icon">
                                <svg class="top">
                                    <use xlink:href="#top">
                    ...
                    <table id="sort" class="w-full shadow-lg rounded-xl"
                           id="myTable">
                        <!-- Table head-->
            ...
            <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                <button
                    ...
                    <span style="padding-top: 1px;">{{__('Кітапты өзгерту')}}
                                    </span>
                </button>
            </td>
    ...
    <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
        <button
            ...
            <span style="padding-top: 1px;">{{__('Кітапты есептен шығару')}}
                                    </span>
        </button>
    </td>
    ...
    <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
        <button
            ...
            <span style="padding-top: 1px;">{{__('Кітапты қайтару')}}
                                    </span>
        </button>
    </td>
    ...
    <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
        <button
            ...
            <span style="padding-top: 1px;">{{__('Кітапқа тапсырыс беру')}}
                                    </span>
        </button>
    </td>
    ...
    @foreach ($books as $book)
        ...
        @if ($searched != true)
            <tr class="hover:bg-gray-200 hover:shadow-md  border-b-[1px] border-[#e4dfdf]">
                ...
                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">
                    ...
                    @if ($book->quantity_count - ($book->rented_count +
                ...
                <td class="px-4 py-4 text-sm leading-5 text-blue-800 whitespace-no-wrap"><a
                        href="{{route('active-reservations')}}">{{$book->reserved_count}}
                </td>
                <td class="px-4 py-4 text-sm leading-5 text-blue-800 whitespace-no-wrap"><a
                            ...
                            <div class="py-1">
                                ...
                                == 3)
                                    <a href="{{route('edit-book', $book->title)}}" tabindex="0"
                                       ...
                                       role="menuitem">
                                        ...
                                        <span class="px-4 py-0">{{__('Кітапты өзгерту')}}</span>
                                    </a>
                                    <a href="{{route('write-off', $book->title)}}" tabindex="0"
                                       class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                       role="menuitem">
                                        <i class="fas fa-level-up-alt mr-[14px] ml-[5px] py-1"></i>
                                        <span class="px-4 py-0">{{__('Кітапты есептен шығару')}}</span>
                                    </a>
                                    ...
                                    <a href="{{route('return-book', $book->title)}}"
                                       ...
                                       role="menuitem">
                                        ...
                                        <span class="px-4 py-0">{{__('Кітапты қайтару')}}</span>
                                    </a>
                                @endif
                                <a href="{{route('reserve-book', $book->title)}}"
                                   ...
                                   role="menuitem">
                                    ...
                                    <span class="px-4 py-0">{{__('Кітапқа тапсырыс беру')}}</span>
                                </a>
                                @if (Auth::user()->type->id == 2 || Auth::user()->type->id
                                == 3)
                                    <button
        ...
        @else
            <tr class="hover:bg-gray-200 hover:shadow-md  border-b-[1px] border-[#e4dfdf]">
                ...
                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">
                    {{$book->book->quantity_count - ($book->book->rented_count +
                    $book->book->reserved_count)}}
                </td>
                <td class="px-4 py-4 text-sm leading-5 text-blue-800 whitespace-no-wrap"><a
                        href="{{route('active-reservations')}}">{{$book->book->reserved_count}}
                </td>
                <td class="px-4 py-4 text-sm leading-5 text-blue-800 whitespace-no-wrap"><a
                        href="{{route('rented-books')}}">{{$book->book->rented_count}}
                </td>
                    ...
                    <div
                        ...
                        <div
                            ...
                            <div class="py-1">
                                ...
                                @if (Auth::user()->type->id == 2 || Auth::user()->type->id
                                == 3)
                                    <a href="{{route('edit-book', $book->book->title)}}"
                                       tabindex="0"
                                       class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                       role="menuitem">
                                        <i class="fas fa-edit mr-[6px] ml-[5px] py-1"></i>
                                ...
                                @if (Auth::user()->type->id == 2 || Auth::user()->type->id
                                == 3)
                                    <form action="{{route('destroy-book', $book->book->id)}}"
                                          method="POST">
                                        @csrf
                                        @method('DELETE')
