...
@section('title')
    ...
    <!-- Title -->
    <title>{{__('Бақылау тақтасы | онлайн кітапхана')}}</title>
...
@section('content')
        ...
        <div style="display:none;" id="myDiv">
                ...
                <h1 class="pl-[30px] pb-[2px]  border-b-[1px] border-[#e4dfdf] ">
                    ...
                    @if (session()->has('approve'))
                        <div id="hideDiv"
                            ...
                            <span class="sr-only">Ақпарат</span>
                            <div>
                                <span class="font-medium">{{__('Сәтті')}}!</span> {{session('approve')}}
                            </div>
                        </div>
                    @endif
            ...
            <div class="pl-[30px] scroll height-dashboard overflow-auto mt-[20px] pb-[30px]">
                <div class="flex flex-row justify-between">
                    <div class="mr-[30px]">
                        <span style="font-size: 1.3em" id="tooltip_1"
                              class="cursor-default uppercase mb-[20px]">{{__('ІС-ШАРАЛАР')}}</span>
                        <!-- Activity Cards -->
                                                                       ...
                                                                       class="text-[#2196f3] hover:text-blue-600">
                                                                        ...
                                                                        <a>
                                                                            ...
                                                                            <a href="{{route('show-book', $rent->book->title)}}">
                                                                                {{__('je')}} {{$rent->librarian->gender->id == 1 ? __('izdao') : __('izdala')}} {{__('knjigu')}}
                                                                                <span
                                                                                    class="font-medium">{{$rent->book->title}} </span>
                                                                            </a>
                                                    ...
                                                    @if ($reservation->reservation->made_by->type->id == 2)
                                                        ...
                                                        <a href="{{route('show-librarian', $reservation->reservation->made_by->username)}}"
                                                           class="text-[#2196f3] hover:text-blue-600">
                                                            ...
                                                            <a>
                                                                @else
                                                                    <a href="{{route('show-admin', $reservation->reservation->made_by->username)}}"
                                                                       class="text-[#2196f3] hover:text-blue-600">
                                                                        {{$reservation->reservation->made_by->name}}
                                                                        <a>
                                                                            @endif

                                                                            {{__('je ')}} {{$reservation->reservation->made_by->gender->id == 1 ? __('rezervisao') : __('rezervisala')}} {{__('knjigu')}}
                                                                            <span
                                                                                class="font-medium">{{$reservation->reservation->book->title}} </span>
                                                                            <a href="{{route('show-student', $reservation->reservation->made_for->username)}}"
                                                                               class="text-[#2196f3] hover:text-blue-600">
                        ...
                        @else
                            ...
                            <div class="mx-[50px]">
                                <div class="w-[400px] flex items-center px-6 py-4 my-4 text-lg bg-[#3f51b5] rounded-lg">
                                    ...
                                    <p class="font-medium text-white">{{__('Соңғы әрекеттер жоқ!')}}</p>
                                </div>
                            </div>
                        ...
                        <div class="mt-[20px]">
                            <table class="w-[560px] table-auto">
                                ...
                                @else
                                    ...
                                    <div class="mx-[50px]">
                                        <div
                                            class="w-[400px] flex items-center px-6 py-4 my-4 text-lg bg-[#3f51b5] rounded-lg">
                                            <svg viewBox="0 0 24 24" class="w-5 h-5 mr-3 text-white sm:w-5 sm:h-5">
                                                <path fill="currentColor"
                                                      d="M11.983,0a12.206,12.206,0,0,0-8.51,3.653A11.8,11.8,0,0,0,0,12.207,11.779,11.779,0,0,0,11.8,24h.214A12.111,12.111,0,0,0,24,11.791h0A11.766,11.766,0,0,0,11.983,0ZM10.5,16.542a1.476,1.476,0,0,1,1.449-1.53h.027a1.527,1.527,0,0,1,1.523,1.47,1.475,1.475,0,0,1-1.449,1.53h-.027A1.529,1.529,0,0,1,10.5,16.542ZM11,12.5v-6a1,1,0,0,1,2,0v6a1,1,0,1,1-2,0Z">
                                                </path>
                                            </svg>
                                            <p class="font-medium text-white">{{__('Кейінге қалдырылған брондау жоқ!')}}</p>
                                        </div>
                                    </div>
                            ...
                            <div class="text-right mt-[5px]">
                                <a href="{{route('active-reservations')}}" class="text-[#2196f3] hover:text-blue-600">
                                    <i style="cursor: default" class="fas fa-calendar-alt mr-[4px]"
                                       aria-hidden="true"></i>
                                    {{__(' Барлығын көрсету')}}
                                </a>
                            </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="text-right">
                                <div class="flex pb-[30px]">
                                    <a class="w-[145px] text-[#2196f3] hover:text-blue-600"
                                       href="{{route('rented-books')}}">
                                        {{__('Жалға алынған кітаптар')}}
                                    </a>
                            ...
                            h-[26px]">
                                ...
                                <div class="flex pb-[30px]">
                                    <a class="w-[145px] text-[#2196f3] hover:text-blue-600"
                                       href="{{route('active-reservations')}}">
                                        {{__('Сақталған кітаптар')}}
                                    </a>
    ...
    <div class="absolutee">
        <div class="font-sans absolute w-full h-full flex justify-center items-center">
            <div class="w-[640px] mx-5">
                <h3 class="text-center text-5xl font-bold font-serif">{{__('Ақпараттық бюллетень')}}</h3>
                <p class="text-center mt-1 mb-2">{{__('Жаңа кітаптардан хабардар болыңыз!')}}</p>
                <form method="GET" class="relative flex items-center my-10" action="{{route('subscribe')}}">
                    <input id="newsletter-input" type="email" name="email" id="email"
                           placeholder="{{__('primjer')}}@email.com"
                           class="w-full bg-transparent py-2 pl-5 pr-20 border-2 border-solid border-black rounded-0 outline-none placeholder:text-black/50"
                           required
                           oninvalid="this.setCustomValidity('Ovo polje je obavezno')"
                    />
                    <button id="newsletter-btn" type="submit"
                            class="absolute h-full right-0 text-white px-5 flex items-center cursor-pointer">
                        <p class="sm:block">{{__('Жіберу')}}</p>
                        <i class="bx bx-chevron-right text-2xl block sm:hidden"></i>
                    </button>
                </form>
                <p id="newsletter-p" class="text-center">*{{__('Сіздің электрондық пошта мекенжайыңыз біз үшін қауіпсіз')}}</p>
            </div>
        </div>
    </div>
    ...
    <div class="absolutee2">
        <div class="font-sans absolute w-full h-full flex justify-center items-center">
            <div class="w-[640px] mx-5">
                <h3 class="text-5xl font-bold font-serif">{{__('Пайдалы сілтемелер')}}</h3>
                <ul>
                    <li class="mt-2 wikipedia-link"><a href="{{route('working-time')}}">{{__('Жұмыс уақыты')}} <i
                                class="fas fa-clock"></i></a></li>
                    <li class="mt-2 wikipedia-link"><a href="{{route('faq')}}">{{__('FAQ-Жиі қойылатын сұрақтар')}}
